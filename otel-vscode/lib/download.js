...
const tmpDir = path.join(os.tmpdir(), `vscode-otel-config-validator-cache-${packageVersion}`);
...
const REPO = 'lightstep/otel-config-validator';
...
async function getAssetFromGithubApi(opts, assetName, downloadFolder) {
    ...
    const downloadOpts = {
        headers: {
            'user-agent': 'vscode-otel-config-validator'
        }
    };
...
async function unzipRipgrep(zipPath, destinationDir, version) {
    if (isWindows) {
    ...
    const expectedName = path.join(destinationDir, `otel-config-validator_${version}`);
    ...
    throw new Error(`Expecting otel-config-validator_${version} or otel-config-validator_${version}.exe unzipped into ${destinationDir}, didn't find one.`);
}
...
module.exports = async opts => {
    ...
    const extension = '.zip';
    const assetName = ['otel-config-validator', opts.version.replace('v', ''), opts.target].join('_') + extension;
    ...
    try {
        const destinationPath = await unzipRipgrep(assetDownloadPath, opts.destDir, opts.version);
        if (!isWindows) {
